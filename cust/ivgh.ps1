...
if($ShowHelp -eq 'Y')
{
    $inveigh.output_queue.Add("[!] Run Stop-Ivgh to stop")  > $null
...
$control_scriptblock =
{
    ...
    while($inveigh.running)
    {
        ...
        if($RunCount)
        {
            ...
            {
                Stop-IvghRunspace "reaching run count"           
            }
        ...
        if($RunTime)
        {
            ...
            {
                Stop-IvghRunspace "reaching run time"
            }
...
<#
.SYNOPSIS
Stop-Ivgh will stop all running Inveigh functions.
#>
...
<#
.SYNOPSIS
Get-Ivgh will get stored Inveigh data from memory.
...
<#
.SYNOPSIS
Watch-Ivgh will enabled real time console output. If using this function through a shell, test to ensure that it doesn't hang the shell.
...
{
    Write-Output "[-] Watch-Ivgh cannot be used with current external tool selection"
}
...
<#
.SYNOPSIS
Clear-Ivgh will clear Inveigh data from memory.
#>
...
if($inveigh)
{
    ...
    {
        Write-Output "[-] Run Stop-Ivgh before running Clear-Ivgh"
    }
...
function ConvertTo-Ivgh
{
    <#
    .SYNOPSIS
    ConvertTo-Ivgh imports Bloodhound computers, groups and session JSON files into $inveigh.enumerate
    ...
    if($inveigh.running -or $inveigh.relay_running)
    {
        Write-Output "Run Stop-Ivgh before importing data with ConvertTo-Ivgh"
        throw
    }
